---
name: policeH
description: The Triple Gate (Haiku edition) — a quality enforcement system for Claude Code. 3 self-drafts + 6 specialized Haiku 4.5 agents across 4 phases. Catches bugs, security holes, and shortcuts before they ship.
---

# /policeH — The Triple Gate Quality Enforcement System
...
**Trust model (v1):** Hybrid. /policeH will RUN pre-setup for read-only analysis and plan-mode reviews, but will REFUSE any destructive operation (Write, Edit, file creation in FORBIDDEN_PATHS, git commit, service restart) until §0 SETUP is complete. Post-SETUP, all operations are permitted subject to the full protocol.
...
sudo chattr +i ~/.claude/skills/policeH/SKILL.md

# 0.5 — Install police-meta-guard hook (blocks Write/Edit to SKILL.md)
cp ~/.claude/skills/policeH/assets/police-meta-guard.sh ~/.claude/hooks/
...
~/.claude/skills/policeH/assets/init-clipboard-pins.sh > ~/.claude/clipboard-pins.json
...
**Detection at runtime (Claude runs this check at every /policeH activation):**
...
lsattr ~/.claude/skills/policeH/SKILL.md 2>/dev/null | grep -q 'i' || SETUP_COMPLETE=false
...
- /policeH runs Phase 1 Drafts 1-3 (read-only, in plan mode — safe)
- /policeH runs Police 1-6 reviews (reads only, safe)
- **/policeH REFUSES Phase 3 (Implementation) with an explicit error message pointing to §0**
- /policeH REFUSES emergency override (§14) entirely
- /policeH REFUSES any Write/Edit touching FORBIDDEN_PATHS
...
FORBIDDEN_PATHS = [
    ...
    "$HOME/.claude/skills/policeH/SKILL.md",
...
A path is forbidden if it matches any entry above (exact or glob). Claude MUST hard-refuse any Write/Edit targeting a forbidden path. The only legitimate modification of `SKILL.md` itself is through the recursive /policeH protocol described in §13.

---

# §2 — Activation (Deterministic Trigger Predicate)

`/policeH` auto-fires when ALL of these are true simultaneously:
...
`/policeH` does NOT auto-fire when:
...
**Manual trigger:** `/policeH` or "with the police" from the user always forces activation, even on otherwise-skipped tasks.
...
**Concurrency isolation:** Per-task random dirs prevent collision between parallel /policeH runs and symlink pre-creation attacks. Every path is scoped to `$STATE_DIR` / `$AUDIT_DIR`, no shared mutable state except the append-only learning log (which uses its own lockfile).
...
**Atomic write rule:** All state.json writes go through `python3 ~/.claude/skills/policeH/assets/state-writer.py "$STATE_DIR" "$JSON"` — writes to `.tmp`, fsync, `os.replace()`. NEVER use a direct `>` redirect on state.json.
...
6. **NEVER use mtime-based "most-recently-modified" resume.** Parallel /policeH tasks would cross-contaminate. mtime ordering was a bug in v3 caught by Police 4 cycle 3.
...
Claude uses the Task tool with `subagent_type: general-purpose` and `model: haiku`. Briefing templates come from §B.1 and §B.2 with placeholders filled. Full verbatim output written to `$AUDIT_DIR/police-<N>-cycle-<M>.md`.
...
```bash
python3 ~/.claude/skills/policeH/assets/scrub.py "$STATE_DIR" $IMPL_FILES
```
...
**Execution:** `python3 ~/.claude/skills/policeH/assets/scrub.py "$STATE_DIR" $IMPL_FILES`
...
**Location in FORBIDDEN_PATHS:** yes (only /policeH itself may append via the documented protocol)
...
```bash
python3 ~/.claude/skills/policeH/assets/append_learning.py \
...
```bash
python3 ~/.claude/skills/policeH/assets/flush_learning_queue.py "$STATE_DIR"
```
...
```bash
PROJECTED_COST=$(~/.claude/skills/policeH/assets/budget-gate.sh) || {
...
- > $90 -> halt and REJECT /policeH until next billing block
...
**Layer 1:** `~/.claude/skills/policeH/SKILL.md` is `chattr +i`. Modification requires `chattr -i`, the change, full /policeH protocol recursively, then `chattr +i`.
...
**Layer 4:** Recursive escape blocked — `/policeH --emergency` (§14) cannot target SKILL.md.

**SETUP state reporting:** Claude runs the §0 detection script at every /policeH activation and reports the status explicitly. If SETUP_COMPLETE=false, destructive operations hard-refuse (not warn).

---

# §14 — Emergency Override (`/policeH --emergency` with GPG signature)

**When legitimate:** Active security incident, crashed critical service, bleeding credential, critical enforcement break.
...
The operator can say "show Police N cycle M" at any time and Claude must Read and display the file verbatim without editing. Softening, omitting, or reframing rejections is an integrity violation.
...
4. **Q4 Alternative implementation:** Would I (Haiku 4.5) write this differently in a measurably better way? -> YES -> REJECT (with named alternative)
...
```
You are a police agent in the /policeH quality enforcement system. You are Claude Haiku 4.5.
